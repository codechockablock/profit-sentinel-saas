...
| POS data adapters | AI mapping service | Pike Creek + Orgill parsers | ✅ Done for Pike Creek |
...
│   ├── pike_creek/
│   │   ├── inventory.py         # Pike Creek/IdoSoft adapter ★
│   │   └── __init__.py
│   ├── orgill/
...
**Pike Creek adapter** — Handles two formats:
...
- **`sales_last_30d`**: Not in any Pike Creek export. `sales_ytd` exists but is annual. Could approximate as `sales_ytd / months_elapsed` or require a separate sales report.
...
The only fixture is `sample_inventory.csv` with 21 synthetic rows. There are no test fixtures generated from real Pike Creek data. The entire pipeline has never been tested against real-world data.
...
**Port priority: LOW for now** — The adapter-per-vendor approach works fine for Pike Creek + known vendors. The AI mapping becomes important only when onboarding many unknown POS systems rapidly.
...
|---|---|---|---|---|
| Pike Creek inventory | `/Users/joseph/Downloads/Reports/` | 17 CSVs | 861K+ | SHLP (36 col) + sales detail |
| Pike Creek inventory | `/Users/joseph/Downloads/custom_1.csv` | 1 CSV | 156K | IdoSoft custom (53 col) |
...
The Rust pipeline needs fields that aren't directly in Pike Creek exports:
...
**Goal:** Run the full pipeline on real Pike Creek data and produce actual insights.
...
- Reads `custom_1.csv` through `PikeCreekInventoryAdapter`
- Converts via bridge to pipeline CSV format
- Writes to `fixtures/pike_creek_real.csv` (sanitized subset: first 1000 rows)
...
| Adapter-per-vendor over AI mapping | Deterministic, testable, handles real edge cases (Pike Creek's "Qty." vs "Qty On Hand") |
