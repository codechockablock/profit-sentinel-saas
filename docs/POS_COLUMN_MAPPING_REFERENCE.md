...
**Confidence:** VERY HIGH (Pike Creek adapter fully implemented)
...
**Confidence:** HIGH (Pike Creek adapter fully implemented, real data validated)
...
| `Qty.`                 | `quantity`            | ⚠ Includes negatives (3,958 in Pike Creek) |
| `Qty On Hand`          | `quantity` (phys)     | Physical count, never negative  |
...
2. **Paladin "Qty." vs "Qty On Hand" distinction.** `Qty.` includes negatives
   ...
   Reference: Pike Creek adapter lines 226-233.

3. **IdoSoft large file sizes.** Pike Creek custom_1.csv has 156K+ rows. Use
...
| `profit-sentinel-rs/python/sentinel_agent/adapters/pike_creek/inventory.py` | Pike Creek/IdoSoft adapter | 35+ |
