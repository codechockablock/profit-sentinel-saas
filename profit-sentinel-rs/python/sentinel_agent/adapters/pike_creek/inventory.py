...
Maps the Pike Creek POS export format to the canonical NormalizedInventory
schema. Pike Creek exports come in two main formats:
...
logger = logging.getLogger("sentinel.adapters.pike_creek")
...
class PikeCreekAdapter(BaseAdapter):
    """Adapter for Pike Creek Community Hardware POS exports."""

    @property
    def name(self) -> str:
        return "Pike Creek Inventory"

    def can_handle(self, path: Path) -> bool:
        """Detect Pike Creek inventory by column signature."""
        path = Path(path)

        if path.is_dir():
            # Look for known Pike Creek filenames
            known_files = [
    ...
    def _check_file(self, path: Path) -> bool:
        """Check if a CSV has Pike Creek column signatures."""
        try:
    ...
    def ingest(self, path: Path, store_id: str = "pike-creek") -> AdapterResult:
        """Parse Pike Creek inventory files.
        ...
        if path.is_dir():
            ...
            if not target:
                return AdapterResult(
                    ...
                    errors=["No compatible Pike Creek CSV found in directory"],
                )
    ...
    def _map_inventory_row(
        ...
        # fall back to "Qty On Hand" (physical count, never negative in Pike Creek data).
