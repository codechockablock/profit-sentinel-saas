...
Usage:
    ...
    python -m sentinel_agent ingest --source /path/to/data --store-id pike-creek
...
def _cmd_ingest(args: argparse.Namespace) -> None:
    ...
    if args.adapter:
        ...
        from .adapters.pike_creek import PikeCreekAdapter

        adapter_map = {
            "orgill": OrgillPOAdapter(),
            "pike_creek": PikeCreekAdapter(),
            "pike-creek": PikeCreekAdapter(),
        }
...
def _run_analysis(result, store_id: str, top_k: int) -> None:
    ...
    if digest.issues:
        print("=" * 64)
        print("  MORNING DIGEST — REAL PIKE CREEK ISSUES")
        print("=" * 64)
        print()
...
def main() -> None:
    ...
    ingest_parser.add_argument(
        "--store-id",
        default="pike-creek",
        help="Store ID for normalized records (default: pike-creek)",
    )
    ingest_parser.add_argument(
        "--adapter",
        help="Force a specific adapter (orgill, pike_creek)",
    )
    ingest_parser.add_argument(
        "--output",
        "-o",
