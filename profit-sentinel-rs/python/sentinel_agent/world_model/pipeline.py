...
def run_integrated_test():
    """
    Demonstrate the full pipeline with all five systems.

    Simulates 3 stores over 8 weeks with realistic patterns
    including seasonal shifts, vendor cost increases, and
    transfer outcomes.
    """
    print("=" * 70)
    print("INTEGRATED PIPELINE TEST — ALL FIVE SYSTEMS")
    print("=" * 70)
    print()
    ...
    print("Simulating 8 weeks of observations across 3 stores...")

    for week in range(8):
        for store_idx, store_id in enumerate(stores):
            timestamp = base_time + week * 7 * day

            # SKU with declining velocity (will trigger dead stock prediction)
            ...
            # SKU with margin erosion (vendor raising costs)
            cost_increase = 8.75 + week * 0.15  # Gradual cost creep
            pipeline.record_observation(
                store_id=store_id,
                entity_id="SKU-5510",
                observation={
            ...
            # Record a vendor delivery (VENDOR_B declining fill rate)
    ...
    # --- Generate predictions ---
    sim_time = base_time + 8 * 7 * day  # End of simulation
    print("Generating predictive interventions...")
    all_interventions = []
    for store_id in stores:
        for entity_id in ["SKU-7742", "SKU-5510"]:
            interventions = pipeline.predict_interventions(
                store_id, entity_id, current_time=sim_time
            )
            all_interventions.extend(interventions)

    print(f"  {len(all_interventions)} interventions generated")
    for i in pipeline.predictive.prioritized_interventions(
        top_k=5, current_time=sim_time
    ):
        print(f"  [{i.intervention_type.value}] {i.store_id}/{i.entity_id}")
        print(f"    {i.description}")
        print(f"    Urgency: {i.urgency:.2f} | Impact: ${i.predicted_impact:,.0f}")
        print(f"    Action: {i.recommended_action}")
        print()

    # --- Simulate feedback (transfer outcome) ---
    print("Recording transfer outcome (feedback loop)...")
    outcome = Outcome(
        outcome_id="OUT-001",
    ...
    reward = pipeline.record_outcome(outcome)
    print(
        f"  Reward: {reward['reward_type']} "
        f"(magnitude: ${reward['reward_magnitude']:,.2f})"
    )
    print()

    # Record a failed prediction for calibration
    outcome2 = Outcome(
        outcome_id="OUT-002",
    ...
    pipeline.record_outcome(outcome2)

    # --- Vendor intelligence ---
    print("Vendor intelligence report:")
    vendor_report = pipeline.vendor_intel.network_vendor_report()
    print(f"  Vendors tracked: {vendor_report['total_vendors']}")
    print(f"  High risk: {len(vendor_report['high_risk_vendors'])}")
    for v in vendor_report["high_risk_vendors"]:
        print(
            f"    - {v['vendor_name']}: risk={v['risk_score']}, "
            f"fill rate={v['fill_rate']}%, "
            f"cost change={v['avg_cost_change']}%"
        )
    print(f"  Active alerts: {vendor_report['active_alerts']}")
    for alert in vendor_report["recent_alerts"][-3:]:
        print(f"    [{alert['type']}] {alert['message']}")
    print()

    # --- Moat snapshot ---
    print("Moat metrics snapshot:")
    snapshot = pipeline.moat_snapshot(n_stores=len(stores), n_skus=2)
    print(f"  Transfer success rate: {snapshot['transfer_success_rate']:.0%}")
    print(f"  Finding accuracy: {snapshot['finding_accuracy_rate']:.0%}")
    print(f"  Prediction accuracy: {snapshot['prediction_accuracy_rate']:.0%}")
    print(f"  Total value recovered: ${snapshot['total_value_recovered']:,.2f}")
    ...
    print("Full pipeline status (for orchestrator context):")
