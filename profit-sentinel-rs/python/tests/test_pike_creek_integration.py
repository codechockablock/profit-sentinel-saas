"""Integration tests: Pike Creek data → Adapter → Bridge → Rust Pipeline → Digest.

These tests verify the complete data flow from real Pike Creek POS exports
...
2. Real data tests (skip if not available) — Use actual Pike Creek CSV files

The fixture files are generated by scripts/generate_fixtures.py from real data:
- pike_creek_sample.csv: 1000 diverse rows (negatives, dead stock, low margin)
- pike_creek_full.csv: All 156K rows
"""
...
_SAMPLE_FIXTURE = _FIXTURES / "pike_creek_sample.csv"
_FULL_FIXTURE = _FIXTURES / "pike_creek_full.csv"
...
@pytest.mark.skipif(
    ...
    reason="Pike Creek sample fixture not generated (run scripts/generate_fixtures.py)",
)
...
class TestFullPipelineIntegration:
    ...
    def test_run_from_adapter_method(self):
        ...
        result = adapter.ingest(_REAL_CUSTOM1, store_id="pike-creek")
