"""Integration tests: Sample Store data → Adapter → Bridge → Rust Pipeline → Digest.

These tests verify the complete data flow from real Sample Store POS exports
...
2. Real data tests (skip if not available) — Use actual Sample Store CSV files

The fixture files are generated by scripts/generate_fixtures.py from real data:
- generic_pos_sample.csv: 1000 diverse rows (negatives, dead stock, low margin)
- generic_pos_full.csv: All 156K rows
"""
...
_SAMPLE_FIXTURE = _FIXTURES / "generic_pos_sample.csv"
_FULL_FIXTURE = _FIXTURES / "generic_pos_full.csv"
...
@pytest.mark.skipif(
    ...
    reason="Sample Store sample fixture not generated (run scripts/generate_fixtures.py)",
)
...
class TestFullPipelineIntegration:
    ...
    def test_run_from_adapter_method(self):
        ...
        result = adapter.ingest(_REAL_CUSTOM1, store_id="default-store")
