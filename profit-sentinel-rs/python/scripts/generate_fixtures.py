#!/usr/bin/env python3
"""Generate real Pike Creek test fixtures from actual data.

This script reads real Pike Creek inventory data, runs it through the
adapter and bridge, and writes pipeline-ready CSV fixtures for testing.

Output:
    tests/fixtures/pike_creek_sample.csv  — 1000 diverse rows for unit tests
    tests/fixtures/pike_creek_full.csv    — All 156K rows for integration tests
...
def main():
    ...
    result = adapter.ingest(REAL_DATA, store_id="pike-creek")
    ...
    full_path = FIXTURE_DIR / "pike_creek_full.csv"
    ...
    sample_path = FIXTURE_DIR / "pike_creek_sample.csv"
