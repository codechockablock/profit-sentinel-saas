...
/// Identifies which stage of the pipeline is executing, for logging.
...
/// The core candidate pipeline trait. Implementors wire up the concrete
/// components (sources, hydrators, filters, scorers, selector, side effects)
/// and get the full execution engine for free via default methods.
#[async_trait]
    ...
    C: Clone + Send + Sync + 'static,
{
    ...
    /// Execute the full pipeline: hydrate query → fetch → hydrate → filter
    /// → score → select → post-select hydrate → post-select filter → side effects.
    async fn execute(&self, query: Q) -> PipelineResult<Q, C> {
        ...
        let (kept_candidates, mut filtered_candidates) = self
            .filter_candidates(&hydrated_query, hydrated_candidates.clone())
            .await;
        ...
        let (mut final_candidates, post_selection_filtered) = self
            .filter_post_selection(&hydrated_query, post_selection_hydrated)
            .await;
        filtered_candidates.extend(post_selection_filtered);
    ...
    /// Run all candidate hydrators in parallel and merge results.
    ...
    /// Run post-selection candidate hydrators in parallel and merge results.
    ...
    async fn filter_candidates(&self, query: &Q, candidates: Vec<C>) -> (Vec<C>, Vec<C>) {
        self.run_filters(query, candidates, self.filters(), PipelineStage::Filter)
            .await
    }

    /// Run post-selection filters sequentially.
    ...
    /// Run all side effects in parallel on a background task.
